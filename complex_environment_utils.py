...
from auxilary import gripper_bd
from auxilary import PandaGripper
...
def parse_isaac_complex_data(path_to_npz='../experiments/pointclouds/complex.npz',
    ...
    total_view = 18 # change to 15 for 006
    ...
    for view_id in range(total_view):
        ...
        _pc1_view2 = get_transform( data[f'view_rot_{chosen_view}'], trans=data[f'view_pos_{chosen_view}'])
        _pc = world2isaac_cam(_pc, _pc1_view2)
        pc1_combined[obj].append(_pc)
        

        # get pc r.t camera frame
        if view_id == chosen_view:
...
if __name__ == "__main__":
    ...
    cat = 'pan'
    idx = 12
    pc, pc_env, obj_trans, obj_quat, pc1, pc1_view, isaac_seed = parse_isaac_complex_data(path_to_npz='../experiments/pointclouds/shelf006.npz',
                                                               cat=cat, idx=idx, env_num=0,
                                                               filter_epsion=1.0)
    obj_mesh = load_shape_for_complex(cat=cat, idx=idx, path='../experiments/composites/shelf006')
